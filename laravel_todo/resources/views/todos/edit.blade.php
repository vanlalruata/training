        ...
        <div class="max-w-2xl mx-auto px-4 sm:px-0">
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="p-8 border-b border-gray-100 bg-gray-50/50">
                    ...
                    <p class="mt-2 text-gray-600">Update your task details and keep moving forward.</p>
                </div>
                ...
                <form action="{{ route('todos.update', $todo) }}" method="POST" class="p-8 space-y-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="description" 
                                  id="description" 
                                  rows="4"
                                  class="block w-full rounded-xl border-gray-200 shadow-sm focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 transition-all duration-200 p-3">{{ old('description', $todo->description) }}</textarea>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div class="space-y-2">
                            ...
                            <select name="priority" 
                                    id="priority"
                                    class="block w-full rounded-xl border-gray-200 shadow-sm focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 transition-all duration-200 p-3">
                                <option value="low" {{ old('priority', $todo->priority) == 'low' ? 'selected' : '' }}>🟢 Low Priority</option>
                                <option value="medium" {{ old('priority', $todo->priority) == 'medium' ? 'selected' : '' }}>🟡 Medium Priority</option>
                                <option value="high" {{ old('priority', $todo->priority) == 'high' ? 'selected' : '' }}>🔴 High Priority</option>
                            </select>
                        </div>
